...
- Hardened release and deployment integrity with main-reachable immutable release candidates, truthful reproducible build metadata, compatible local Docker builds, repeatable private Cloudflare deploys, and fail-closed FakeCo rollback that restores pre-update SQLite state.
- Preserved web realtime and account state by advancing cursors only after successful delivery, isolating browser storage failures, discarding cross-workspace uploads, sending owned settings fields, and trapping modal focus.
- Hardened desktop lifecycle behavior with strict server readiness validation, deduplicated expiring OAuth attempts, Windows unread overlay replay, and renderer-ready Quick Compose delivery.
- Completed the OpenAPI and TypeScript SDK contract with bearer authentication, response schemas, query controls, agent progress, notification and thread state types, corrected message references, and generated-contract drift checks.
- Serialized Postgres authorization, moderation, nonce, delete, update, and attachment races so revoked members cannot complete in-flight writes and concurrent retries recover without leaving transactions aborted.
- Hardened app callbacks and workspace-scoped APIs by blocking private-network destinations, bounding asynchronous event delivery and subscription counts, filtering private events by recipient, enforcing bot workspace isolation, and preserving omitted profile and notification fields.
