...
**Repo:** `CatabolicSolutions/clickclack`, branch `cognitive-os` (canonical commit **`07124eb`**)
...
- `git fetch origin` first. Canonical state = commit **`07124eb`**; BOTH
  ...
  dirty state; hard-reset/checkout the fetched `07124eb` before starting.
...
- **Recent commits (all on the fork):** `138771d` phase-bc feature set
  ...
  **blank-page fix**: `paths.base='/logos'` in `apps/logos/svelte.config.js`
  ...
  assets 200 under `/logos/_app/...`) → `07124eb` this directive.
