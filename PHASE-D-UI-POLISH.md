...
**Repo:** `CatabolicSolutions/clickclack`, branch `cognitive-os` (canonical state: latest commit on these branches — they are kept identical)
...
- `git fetch origin` first. Canonical state = the latest commit on `cognitive-os` / `catabolicsolutions-logos-phase-bc` (kept identical); BOTH
  ...
  dirty state; hard-reset/checkout `origin/cognitive-os` (fetched) before starting.
...
- **Recent commits (all on the fork):** `138771d` phase-bc feature set
  ...
  **blank-page fix**: `paths.base='/logos'` in `apps/logos/svelte.config.js`
  ...
  assets 200 under `/logos/_app/...`) → this directive (Phase D).
