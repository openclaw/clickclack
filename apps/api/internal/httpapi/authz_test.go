...
func TestHTTPNotFoundPreservesAPIAndAssetBoundaries(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing web manifest", method: http.MethodGet, path: "/manifest-missing.webmanifest", wantStatus: http.StatusNotFound, wantType: "text/plain"},
		{name: "existing service worker", method: http.MethodGet, path: "/service-worker.js", wantStatus: http.StatusOK, wantType: "text/javascript"},
		{name: "existing web manifest", method: http.MethodGet, path: "/manifest.webmanifest", wantStatus: http.StatusOK, wantType: "text/plain"},
