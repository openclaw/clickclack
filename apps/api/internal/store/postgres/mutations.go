...
func (s *Store) UpdateWorkspace(ctx context.Context, input store.UpdateWorkspaceInput) (store.Workspace, store.Event, error) {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE workspaces SET name = $1, slug = $2, icon_url = $3 WHERE id = $4`, name, workspaceSlug, iconURL, input.WorkspaceID); err != nil {
...
func (s *Store) TransferWorkspaceOwnership(ctx context.Context, input store.TransferWorkspaceOwnershipInput) (store.Workspace, store.Event, error) {
	...
	if err := tx.QueryRowContext(ctx, `SELECT role FROM workspace_members WHERE workspace_id = $1 AND user_id = $2`, input.WorkspaceID, targetID).Scan(&targetRole); err != nil {
