...
func (s *Store) CreateMessage(ctx context.Context, input store.CreateMessageInput) (store.Message, store.Event, error) {
	...
	if err := lockMemberWriteAuthorizationTx(ctx, tx, workspaceID, input.AuthorID); err != nil {
		return store.Message{}, store.Event{}, err
	}
	var nonceConflict bool
	if existing, recovered, err := recoverChannelMessageByClientNonceTx(ctx, tx, input, body, nonce, kind, quotedID); errors.Is(err, store.ErrClientNonceConflict) {
		nonceConflict = true
	} else if err != nil {
		return store.Message{}, store.Event{}, err
	} else if recovered {
	...
	role, err := requireCanPostAuthorizationLockedTx(ctx, tx, workspaceID, input.ChannelID, input.AuthorID)
	if err != nil {
		return store.Message{}, store.Event{}, err
	}
	if nonceConflict {
	...
	inserted, err := qtx.InsertChannelMessage(ctx, storedb.InsertChannelMessageParams{
		ID:                 id,
...
func (s *Store) CreateThreadReply(ctx context.Context, input store.CreateThreadReplyInput) (store.Message, store.ThreadState, []store.Event, error) {
	...
	if err := lockMemberWriteAuthorizationTx(ctx, tx, root.WorkspaceID, input.AuthorID); err != nil {
		return store.Message{}, store.ThreadState{}, nil, err
	}
	...
	var nonceConflict bool
	if existing, state, recovered, err := recoverThreadReplyByClientNonceTx(ctx, tx, root.ID, input.AuthorID, body, nonce, quotedID); errors.Is(err, store.ErrClientNonceConflict) {
		nonceConflict = true
	} else if err != nil {
		return store.Message{}, store.ThreadState{}, nil, err
	} else if recovered {
	...
	if root.DirectConversationID != "" {
		if err := requireCanSendDirectLockedTx(ctx, tx, root.WorkspaceID, input.AuthorID); err != nil {
			return store.Message{}, store.ThreadState{}, nil, err
		}
	} else {
		role, err = requireCanPostAuthorizationLockedTx(ctx, tx, root.WorkspaceID, root.ChannelID, input.AuthorID)
		if err != nil {
			return store.Message{}, store.ThreadState{}, nil, err
		}
	}
	...
	inserted, err := qtx.InsertThreadReply(ctx, storedb.InsertThreadReplyParams{
		ID:                   id,
