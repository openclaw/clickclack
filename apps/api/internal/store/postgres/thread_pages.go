...
func (s *Store) GetThreadPage(ctx context.Context, rootMessageID, userID string, req store.ThreadPageRequest) (store.ThreadPage, error) {
	...
	query := func(lower, upper int64, descending bool, limit int) ([]store.Message, error) {
		...
		rows, err := s.q.ListThreadReplyPage(ctx, storedb.ListThreadReplyPageParams{RootID: root.ID, LowerSeq: sql.NullInt64{Int64: lower, Valid: true}, UpperSeq: sql.NullInt64{Int64: upper, Valid: true}, DescendingOrder: int32(direction), PageLimit: int32(limit)})
		if err != nil {
			return nil, err
		}
