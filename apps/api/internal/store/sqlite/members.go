...
func (s *Store) ListWorkspaceMemberPage(ctx context.Context, workspaceID, actorUserID string, page store.WorkspaceMemberPageRequest) (store.WorkspaceMemberPage, error) {
	...
	totalByRole, err := sqliteWorkspaceMemberRoleCounts(ctx, tx, workspaceID, req)
	if err != nil {
		return store.WorkspaceMemberPage{}, err
	}
...
func sqliteWorkspaceMemberRoleCounts(ctx context.Context, tx *sql.Tx, workspaceID string, req store.WorkspaceMemberPageRequest) (*store.WorkspaceMemberRoleCounts, error) {
