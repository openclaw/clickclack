...
// snapshot yet is not an error: it returns an empty record (ChannelID set) so
// the meter renders an "unknown" state instead of failing.
func (s *Store) GetChannelRuntime(ctx context.Context, channelID string) (store.ChannelRuntime, error) {
	const q = `SELECT channel_id, default_model, default_thinking, model, thinking,
		...
		FROM channel_runtime WHERE channel_id = ?`
...
// picker override (override_model/override_thinking) untouched.
...
// leaving the bridge-owned snapshot untouched.
