...
func TestSearchWorkspaceFTSMigrationPreservesAndScopesRows(t *testing.T) {
	...
	// Current channel queries include later independent channel columns. Apply
	// those schema additions early while leaving the FTS migration under test
	// unapplied, and record them so Migrate does not run them twice below.
	for _, name := range []string{"0037_managed_channels.sql", "0039_channel_description.sql"} {
