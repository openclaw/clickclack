...
func TestSidebarPreferencesReadReturnsEveryWorkspace(t *testing.T) {
	...
	user, err := st.CreateUser(ctx, store.CreateUserInput{DisplayName: "Wide User", Email: "sidebar-wide-sqlite@example.com"})
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i <= store.MaxSidebarChannelOrderWorkspaces; i++ {
		workspace, err := st.CreateWorkspace(ctx, store.CreateWorkspaceInput{Name: "Wide", Slug: "wide-" + strconv.Itoa(i)}, user.ID)
		if err != nil {
			t.Fatal(err)
		}
