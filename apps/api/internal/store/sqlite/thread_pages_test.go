...
func TestGetThreadLatestReturnsBoundedChronologicalWindow(t *testing.T) {
	...
	threadResult1, err := st.GetThreadPage(ctx, root.ID, owner.ID, store.ThreadPageRequest{MessagePageRequest: store.MessagePageRequest{Limit: 2}})

	threadRoot, first := threadResult1.Root, threadResult1.Replies
	if err != nil {
		t.Fatal(err)
	}
	...
	threadResult2, err := st.GetThreadPage(ctx, root.ID, owner.ID, store.ThreadPageRequest{MessagePageRequest: store.MessagePageRequest{Limit: 2}, Latest: true})
	latest, state := threadResult2.Replies, threadResult2.ThreadState
	if err != nil {
		t.Fatal(err)
	}
