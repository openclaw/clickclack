...
import { api, applyDefaultFetchTimeout, DEFAULT_FETCH_TIMEOUT_MS } from "./api.ts";
...
test("api() aborts a hung fetch with the default timeout", async () => {
  const { result } = await withMockedFetch(hangUntilAborted, 20, async () => {
...
test("applyDefaultFetchTimeout aborts a hung slash-command-style fetch", async () => {
  const originalFetch = globalThis.fetch;
  const originalTimeout = AbortSignal.timeout;
  const timeoutArgs: number[] = [];
